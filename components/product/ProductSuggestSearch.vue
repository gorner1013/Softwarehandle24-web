...
<template>
  <div
    class="p-3 h-14 text-sm flex items-center gap-3 hover:bg-gray-800 cursor-pointer transition duration-300 bg-white"
  >
    <div class="rounded-md border border-gray-300 overflow-hidden flex-none">
      <img
        ref="imageElement"
        loading="lazy"
    ...
    <div class="flex items-center justify-between overflow-hidden gap-5 grow">
      ...
      <div class="flex-none text-right">
        <SharedPrice
          ...
          class="justify-end text-black"
          :value="unitPrice"
        >
          <template #beforePrice>
            <p v-if="displayFrom">{{ $t('product.price.from') }}</p>
          </template>
        </SharedPrice>
        <ProductUnits
